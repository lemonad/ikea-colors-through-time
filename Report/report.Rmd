...
for (year in years) {
  ...
  pooled_means[length(pooled_means) + 1] <- pooled_mean
  ...
  confidence_intervals[length(confidence_intervals) + 1] <- (
    1.96 * pooled_sd * ninv
  )
}
...
mean_df %>%
  ...
  geom_vline(xintercept = 2005, linetype = 2, color = "gray") +
  annotate(
    "text",
    label = "1991",
    x = 1991,
...
catalogs in the 1990's, it seems non-trivial to pick this up by looking at color
alone. This says something about IKEA's visual style. That IKEA caters to
everyone and not everyone wants the same thing at the same time does not help
with picking up trends either, which is made very obvious by the 1969 catalog:
